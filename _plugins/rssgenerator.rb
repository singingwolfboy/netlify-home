  ...
  class RssGenerator < Generator
    ...
    def generate(site)
      ...
      rss = RSS::Maker.make("2.0") do |maker|
        maker.channel.title = config['name']
        maker.channel.link = config['url']
        maker.channel.description = config['description'] || "RSS feed for #{site.config['name']}"
        maker.channel.author = config["author"]
        maker.channel.updated = site.posts.map { |p| p.date  }.max
        maker.channel.copyright = config['copyright']

        post_limit = config['post_limit'].nil? ? site.posts.count : config['post_limit'] - 1
      ...
      rss_path = ensure_slashes(config['path'] || "/")
      rss_name = config['filename'] || "rss.xml"
